...
# Grammar Dict
from exporter.grammar_dict.grammar_dict_ru import (
    generate_grammar_row_html, 
    generate_html_from_lookup, 
    ProgData
)
# DB Rebuild
from scripts.build.db_rebuild_from_tsv_ru_sbs import (
    get_tsv_files,
    make_russian_table_data
)
# GoldenDict Ru
...
class TestGrammarDictRu:
    def test_generate_grammar_row_html_basic(self):
        """Test simple grammar row generation."""
        data = ("word", "noun", "masc nom sg")
        html = generate_grammar_row_html(data)
        assert "<td>word</td>" in html
        assert "<b>noun</b>" in html
        assert "<td>masc</td>" in html
        assert "<td>nom</td>" in html
        assert "<td>sg</td>" in html

    def test_generate_grammar_row_html_reflx(self):
        """Test reflexsive grammar row."""
        data = ("word", "verb", "reflx past 3rd")
        html = generate_grammar_row_html(data)
        assert "<td>reflx past</td>" in html
        assert "<td>3rd</td>" in html

    @patch('exporter.grammar_dict.grammar_dict_ru.load_abbreviations_dict')
    @patch('exporter.grammar_dict.grammar_dict_ru.Template')
    @patch('exporter.grammar_dict.grammar_dict_ru.CSSManager')
    def test_generate_html_from_lookup(self, mock_css, mock_template, mock_load_abbr, mock_db_session, mock_project_paths):
        """Test generation of HTML from lookup data."""
        # Setup mocks
        g = MagicMock(spec=ProgData)
        ...
        mock_db_session.query.return_value.filter.return_value.all.return_value = [mock_lookup]
        
        # Mock Template
        mock_template_instance = mock_template.return_value
        mock_template_instance.render.return_value = "<header>"
        
        # Mock CSS
        mock_css.return_value.update_style.return_value = "<header><style>"
...
class TestDBRebuildRuSbs:
    ...
    def test_make_russian_table_data(self, mock_db_session):
        ...
        with patch('scripts.build.db_rebuild_from_tsv_ru_sbs.get_tsv_files') as mock_get_files, \
             patch('scripts.build.db_rebuild_from_tsv_ru_sbs.read_tsv_files') as mock_read_files:
            
            mock_get_files.return_value = [Path("fake.tsv")]
            mock_read_files.return_value = [(columns, row)]
            
            make_russian_table_data(mock_dps_paths, mock_db_session)
            ...
            obj = args[0]
            assert obj.id == "1"
            assert obj.ru_meaning == "значение"
...
class TestGoldenDictRu:
    ...
    def test_main_execution(self, mock_config, mock_limit, mock_size, mock_prep, mock_gen_dpd, mock_gv):
...
class TestTprExporterRu:
    ...
    def test_main(self, mock_clean_hw, mock_copy_zip, mock_updater, mock_copy_db, mock_write_tsv, 
                  mock_roots, mock_spelling, mock_deconstruct, mock_gen_tpr, mock_gv, mock_config):
...
class TestGuiMain:
    ...
    def test_app_initialization(self, mock_dps, mock_bold, mock_transl, mock_filter, mock_sandhi2, mock_sandhi1,
                                mock_tests, mock_p2add, mock_p2auto, mock_p2pre, mock_p1add, mock_p1auto, mock_global,
                                mock_toolkit, mock_page, mock_server):
