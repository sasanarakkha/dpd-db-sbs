...
def test_natural_sort_key_orders_dhp2_before_dhp10() -> None:
    assert sorted(["DHP10", "DHP1", "DHP2"], key=natural_sort_key) == [
        "DHP1",
        "DHP2",
        "DHP10",
    ]
...
def test_should_overwrite_existing_respects_force_and_prefer_earlier() -> None:
    assert should_overwrite_existing("DHP10", "DHP2", force=False, prefer_earlier=True)
    assert not should_overwrite_existing(
        "DHP2", "DHP10", force=False, prefer_earlier=True
    )
    assert not should_overwrite_existing(
        "DHP10", "DHP2", force=False, prefer_earlier=False
    )
    assert should_overwrite_existing("DHP2", "DHP10", force=True, prefer_earlier=False)
