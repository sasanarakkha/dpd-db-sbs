...
class TestBoldComponentInToken:
    def test_direct_match(self, db_session: Session):
        assert bold_component_in_token("viharantaṃ", "viharantaṃ", 999999, db_session) == "<b>viharantaṃ</b>"

    def test_strip_m_match(self, db_session: Session):
        # mattaṃ → matta is a substring of āmattaññuṃ
        assert bold_component_in_token("c'āmattaññuṃ", "mattaṃ", 999999, db_session) == "c'ā<b>matta</b>ññuṃ"

    def test_compound_prefix_no_apostrophe(self, db_session: Session):
        # hīna is a direct substring of hīnavīriyaṃ
        assert bold_component_in_token("hīnavīriyaṃ", "hīna", 999999, db_session) == "<b>hīna</b>vīriyaṃ"

    def test_apostrophe_split_left_subha(self, db_session: Session):
        # subhaṃ → consonant frame subh → matches left part 'subh'
        assert bold_component_in_token("subh'ānupassiṃ", "subhaṃ", 999999, db_session) == "<b>subh</b>'ānupassiṃ"
    ...
    def test_apostrophe_split_right_anupassi(self, db_session: Session):
        ...
        assert bold_component_in_token("subh'ānupassiṃ", "anupassī", 999999, db_session) == "subh'<b>ānupassiṃ</b>"

    def test_apostrophe_split_left_ca(self, db_session: Session):
        # ca → consonant frame c → matches left part 'c'
        assert bold_component_in_token("c'āmattaññuṃ", "ca", 999999, db_session) == "<b>c</b>'āmattaññuṃ"
    ...
    def test_apostrophe_split_right_amatta(self, db_session: Session):
        ...
        assert bold_component_in_token("c'āmattaññuṃ", "amattaññuṃ", 999999, db_session) == "c'<b>āmattaññuṃ</b>"

    def test_rukkham_direct_match(self, db_session: Session):
        assert bold_component_in_token("rukkhaṃ'va", "rukkhaṃ", 999999, db_session) == "<b>rukkhaṃ</b>'va"

    def test_iva_right_side(self, db_session: Session):
        # iva → consonant frame iv → doesn't match left 'rukkhaṃ' → bold right
        assert bold_component_in_token("rukkhaṃ'va", "iva", 999999, db_session) == "rukkhaṃ'<b>va</b>"
...
class TestBoldWordInVerse:
    def test_simple_word(self, db_session: Session):
        verse = "manasā ce paduṭṭhena"
        assert bold_word_in_verse(verse, "manasā", "manasā", 999999, db_session) == "<b>manasā</b> ce paduṭṭhena"

    def test_multiple_occurrences(self, db_session: Session):
        verse = "akkocchi maṃ avadhi maṃ"
        assert bold_word_in_verse(verse, "maṃ", "maṃ", 999999, db_session) == "akkocchi <b>maṃ</b> avadhi <b>maṃ</b>"

    def test_apostrophe_token_left_bold(self, db_session: Session):
        verse = "subh'ānupassiṃ viharantaṃ"
        result = bold_word_in_verse(verse, "subh'ānupassiṃ", "subhaṃ", 999999, db_session)
        assert result == "<b>subh</b>'ānupassiṃ viharantaṃ"

    def test_apostrophe_token_right_bold(self, db_session: Session):
        verse = "subh'ānupassiṃ viharantaṃ"
        result = bold_word_in_verse(verse, "subh'ānupassiṃ", "anupassī", 999999, db_session)
    ...
    def test_matta_in_camattannhum(self, db_session: Session):
        ...
        result = bold_word_in_verse(verse, "c'āmattaññuṃ", "mattaṃ", 999999, db_session)
        assert result == "bhojanamhi c'ā<b>matta</b>ññuṃ"

    def test_no_match_returns_original(self, db_session: Session):
        verse = "akkocchi maṃ avadhi maṃ"
        assert bold_word_in_verse(verse, "notinverse", "notinverse", 999999, db_session) == verse


class TestCollectAllIds:
    def test_simple_option_no_components(self):
        option = {"key": "123_0", "id": 123, "pali": "dhammā", "ai_score": 10, "components": []}
        assert collect_all_ids(option, "dhammā") == [(123, "dhammā", "dhammā")]

    def test_decon_key_skipped(self):
        option = {"key": "decon_foo_0", "id": "", "pali": "foo", "ai_score": 10, "components": []}
        assert collect_all_ids(option, "foo") == []

    def test_recurses_into_compound_components(self):
        option = {
            "key": "71345_3", "id": 71345, "pali": "hīnavīriyaṃ", "ai_score": 10,
            "compound_type": "kammadhāraya",
            "components": [
                [{"key": "71324_0", "id": 71324, "pali": "hīna", "ai_score": 0, "compound_type": "", "components": []}],
                [{"key": "69911_0", "id": 69911, "pali": "vīriya", "ai_score": 0, "compound_type": "", "components": []}],
            ],
        }
    ...
    def test_decon_top_still_recurses_children(self):
        option = {
            "key": "decon_cāmattaññuṃ_1", "id": "", "pali": "cāmattaññuṃ", "ai_score": 10,
            "components": [
                [{"key": "25662_default", "id": 25662, "pali": "ca", "ai_score": 10, "components": []}],
                [{"key": "8487_2", "id": 8487, "pali": "amattaññuṃ", "ai_score": 10, "components": []}],
            ],
        }
    ...
    def test_deeply_nested(self):
        option = {
            "key": "8487_2", "id": 8487, "pali": "amattaññuṃ", "ai_score": 10,
            "compound_type": "kammadhāraya",
            "components": [
                [{"key": "35345_0", "id": 35345, "pali": "na", "ai_score": 0, "compound_type": "", "components": []}],
                [{"key": "50951_0", "id": 50951, "pali": "mattaññū", "ai_score": 0,
                  "compound_type": "kammadhāraya",
                  "components": [
                      [{"key": "50938_0", "id": 50938, "pali": "mattaṃ", "ai_score": 0, "compound_type": "", "components": []}],
                      [{"key": "28965_0", "id": 28965, "pali": "ñū", "ai_score": 0, "compound_type": "", "components": []}],
                  ]}],
            ],
        }
        assert [r[0] for r in collect_all_ids(option, "amattaññuṃ")] == [8487, 35345, 50951, 50938, 28965]
    ...
    def test_collect_all_ids_no_recurse_uttara_type(self):
        ...
        option = {
            "key": "14659_0", "id": 14659, "pali": "uttara", "ai_score": 0,
            "compound_type": "",
            "pos": "adj",
            "components": [
                [{"key": "missing_ud", "id": "", "pali": "ud", "compound_type": "", "ai_score": 0, "components": []}],
                [{"key": "30078_0", "id": 30078, "pali": "tara", "compound_type": "", "ai_score": 0, "components": []}],
            ],
        }
    ...
    def test_collect_all_ids_anuttara_atomic_no_components(self):
        ...
        option = {
            "key": "4524_0", "id": 4524, "pali": "anuttara", "ai_score": 10,
            "compound_type": "kammadhāraya",
            "components": [],
        }
        assert collect_all_ids(option, "anuttaraṃ") == [(4524, "anuttara", "anuttaraṃ")]
    ...
    def test_collect_all_ids_akatapāpa_breaks(self):
        ...
        na_id = 35345
        option = {
            "key": f"{akatapāpa_id}_0", "id": akatapāpa_id, "pali": "akatapāpa", "ai_score": 10,
            "compound_type": "kammadhāraya",
            "components": [
                [{"key": f"{na_id}_0", "id": na_id, "pali": "na", "compound_type": "", "ai_score": 0, "components": []}],
                [{"key": f"{katapāpa_id}_0", "id": katapāpa_id, "pali": "katapāpa",
                  "compound_type": "kammadhāraya",
                  "ai_score": 0,
                  "components": [
                      [{"key": f"{kata_id}_0", "id": kata_id, "pali": "kata", "compound_type": "", "ai_score": 0, "components": []}],
                      [{"key": f"{pāpa_id}_0", "id": pāpa_id, "pali": "pāpa", "compound_type": "", "ai_score": 0, "components": []}],
                  ]}],
            ],
        }
...
class TestBoldComponentStemMatch:
    def test_stem_match_compound_component(self, db_session: Session):
        ...
        assert bold_component_in_token("daḷhaparakkamā", "parakkama", 999999, db_session) == "daḷha<b>parakkamā</b>"
