...
def test_tpr_index_check_not_last(tmp_path):
    ...
    mock_data = [{"name": "Other 1"}, {"name": "DPD with Russian"}, {"name": "Other 2"}]
    with open(json_path, "w") as f:
        json.dump(mock_data, f)

    script_path = Path("scripts/rus_exporter/check_tpr_download_index.py")
    result = subprocess.run(
        ["python3", str(script_path), "--test-path", str(submodule_path)],
        capture_output=True,
        text=True,
    )
...
def test_tpr_index_check_is_last(tmp_path):
    ...
    mock_data = [{"name": "Other 1"}, {"name": "DPD with Russian"}]
    with open(json_path, "w") as f:
        json.dump(mock_data, f)
...
def test_tpr_index_check_absent(tmp_path):
    ...
    mock_data = [{"name": "Other 1"}, {"name": "Other 2"}]
    with open(json_path, "w") as f:
        json.dump(mock_data, f)
