...
try:
    results = (
        session.query(
            ...
            case((DpdAudio.male1 is not None, True), else_=False).label("has_male1"),
            case((DpdAudio.male2 is not None, True), else_=False).label("has_male2"),
            case((DpdAudio.female1 is not None, True), else_=False).label(
                "has_female1"
            ),
        )
        .limit(5)
