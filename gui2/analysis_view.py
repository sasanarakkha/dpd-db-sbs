...
class AnalysisRow(ft.Container):
    ...
    def populate_dropdown(self):
        ...
        for detail in self.details_list:
            ...
            symbol = {"complete": "✔", "semi-complete": "◑", "incomplete": "✘"}.get(
                comp, ""
            )
            ...
            options.append(ft.dropdown.Option(key=detail["key"], text=label))
        ...
        if selected_key == "Manual":
            ...
            for detail in self.details_list:
                if detail["key"] == selected_key:
                    ...
                    if self.parent_view and hasattr(
                        self.parent_view, "analysis_results"
                    ):
    ...
    def _update_parent_selection(self, selected_key: str):
        ...
        def find_and_update(data_list_or_obj):
            if isinstance(data_list_or_obj, list):
                ...
                if any(
                    isinstance(i, dict) and i.get("key") == selected_key
...
class AnalysisView(ft.Column):
    def __init__(self, page: ft.Page, toolkit: ToolKit):
        ...
        self.translation_label = ft.Text(
            "", weight=ft.FontWeight.BOLD, italic=True, selectable=True
        )
        ...
        self.export_row = ft.Row(
            controls=[self.export_csv_btn, self.save_md_btn], visible=False
        )
    ...
    def load_sutta(self, e: ft.ControlEvent) -> None:
        ...
        if results:
            ...
            self.page.snack_bar = ft.SnackBar(
                ft.Text("Sutta title not found in Tipitaka text.")
            )
            self.page.snack_bar.open = True
        self.page.update()
    ...
    def _execute_analysis(self, ai: bool = False) -> None:
        ...
        self.translation_label.value = (
            "Analyzing..." if not ai else "AI is thinking (requesting OpenRouter)..."
        )
        ...
        try:
            ...
            if ai:
                ...
                self.translation_label.value = (
                    f"Translation: {result_obj.get('translation', '')}"
                )
                self.literal_label.value = (
                    f"Literal: {result_obj.get('literal_translation', '')}"
                )
        ...
        for result in self.analysis_results:
            ...
            if not data_list:
                self.results_container.controls.append(
                    AnalysisRow(word, [], parent_view=self)
                )
                continue
            ...
            selected_entry = next(
                (d for d in data_list if d["key"] == selected_key), data_list[0]
            )
    ...
    def _render_components(self, entry: dict) -> None:
        ...
        if "components" not in entry or not entry["components"]:
            return
        ...
        for comp_options in entry["components"]:
            ...
            comp_row = AnalysisRow(
                f"- {comp_word}", comp_options, is_component=True, parent_view=self
            )
            ...
            selected_comp_entry = next(
                (d for d in comp_options if d["key"] == selected_comp_key),
                comp_options[0],
            )
    ...
    def retranslate(self, e: ft.ControlEvent) -> None:
        ...
        text = self.input_field.value
        if not text or not self.analysis_results:
            return
        ...
        try:
                    ...
                    for entry in word_obj["data"]:
                        if entry.get("selected"):
                            ...
                            if "components" in new_entry:
                                new_entry["components"] = [
                                    filter_selected(cl)
                                    for cl in new_entry["components"]
                                ]
            ...
            if response.content:
                ...
                self.translation_label.value = (
                    f"Refined Translation: {ai_data.get('translation', '')}"
                )
                self.literal_label.value = (
                    f"Refined Literal: {ai_data.get('literal_translation', '')}"
                )
            else:
    ...
    def export_csv(self, e: ft.ControlEvent) -> None:
        ...
        filepath = (
            Path(self.toolkit.project_paths.temp_dir)
            / f"analysis_{datetime.now().strftime('%H%M%S')}.csv"
        )
    ...
    def save_md(self, e: ft.ControlEvent) -> None:
        ...
        filepath = (
            Path(self.toolkit.project_paths.temp_dir)
            / f"analysis_{datetime.now().strftime('%H%M%S')}.md"
        )
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(
                "| ID | Word | Grammar | Meaning | Construction | Root |\n| :--- | :--- | :--- | :--- | :--- | :--- |\n"
            )
            for r in data:
                f.write(
                    f"| {r['ID']} | {r['Word']} | {r['Grammar']} | {r['Meaning']} | {r['Construction']} | {r['Root']} |\n"
                )
