...
def find_all_bold_words(g):
    ...
    # Support nested attribute access for fields like "sbs.class_example"
    if "." in g.field:
        parent, child = g.field.split(".", 1)
        parent_obj = getattr(g.i, parent, None)
        value = getattr(parent_obj, child, "") if parent_obj else ""
    else:
        value = getattr(g.i, g.field, "")
    g.bold_words = re.findall(r"<b>.+?</b>", value)
    ...
    if g.bold_word[3] == g.bold_word[4]:
        ...
        if g.pass_no == "pass2":
            if "." in g.field:
                parent, child = g.field.split(".", 1)
                parent_obj = getattr(g.i, parent, None)
                sentence = getattr(parent_obj, child, "") if parent_obj else ""
            else:
