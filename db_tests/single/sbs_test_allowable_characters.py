...
class AllowableCharacters:
    ...
    star = [
        "\\*"
    ]  #   to show ṇ anubandha in grammatical terms and imaginary word forms
    ...
    verbs = ["caus", "deno", "desid", "impers", "intens", "pass"]
    ...
    origins = ["pass1", "pass2", "dps", "sandhi", "cpd", "dps", "ncped"]
    ...
    lemma_1_allowed = pali_alphabet + fullstop + space + digits

    lemma_2_allowed = pali_alphabet + fullstop + space + digits + dash

    pos_allowed = english_alphabet
    ...
    derived_from_allowed = pali_alphabet + space + root

    neg_allowed = neg + space

    verb_allowed = verbs + space + comma

    trans_allowed = trans

    plus_case_allowed = plus_case + and_or + space
    ...
    root_sign_allowed = pali_alphabet + space + plus + star
    ...
    family_root_allowed = pali_alphabet + root + space

    family_word_allowed = pali_alphabet + digits

    family_compound_allowed = pali_alphabet + digits + space
    ...
    derivative_allowed = derivatives

    suffix_allowed = pali_alphabet + star
    ...
    compound_type_allowed = compound_types + space + greater_than

    compound_construction_allowed = pali_alphabet + space + plus + bold + new_line
    ...
    ant_syn_var_allowed = pali_alphabet + space + comma
    ...
    stem_allowed = pali_alphabet + exclamation + star + dash

    pattern_allowed = patterns + space
...
def main(table_to_check):
    ...
    if table_to_check == "SBS":
        ...
        tests_data = a.ru_data
    else:
        print("[red]Invalid table to check. Use 'SBS' or 'Russian'.")
        return
...
def check_root_db():
    ...
    for root_data in a.roots_ru_data:
        # setup variables
