...
def render_dpd_xhtml(pth: ProjectPaths, rupth: RuPaths):
    ...
    for counter, i in enumerate(deconstructor_db):
        if bool(set(i.lookup_key) & all_words_set):
            ...
            entry = render_deconstructor_entry(jinja_env, id_counter, i)
            letter_dict[first_letter] += [entry]
            id_counter += 1
        if counter % 5000 == 0:
...
def render_ebook_entry(
    ...
    # clean up for html
    for attr in ["root_base", "construction", "sanskrit", "compound_type", "phonetic", 
                 "example_1", "example_2", "sutta_1", "sutta_2", "commentary", "notes", "cognate"]:
        val = getattr(i, attr)
    ...
    examples = render_example_templ(jinja_env, i)
...
def render_grammar_templ(jinja_env, i: DpdHeadword) -> str:
...
def render_example_templ(jinja_env, i: DpdHeadword) -> str:
...
def render_deconstructor_entry(jinja_env, counter: int, i: Lookup) -> str:
...
def render_ebook_letter_templ(jinja_env, letter: str, entries: str) -> str:
...
def save_abbreviations_xhtml_page(rupth: RuPaths, id_counter):
    ...
    for i in abbreviations_list:
        ...
        abbreviation_entries += [render_abbreviation_entry(jinja_env, id_counter, i)]
        id_counter += 1

    entries = "".join(abbreviation_entries)
    xhtml = render_ebook_letter_templ(jinja_env, "Сокращения", entries)
    with open(rupth.epub_abbreviations_path, "w") as f:
        f.write(xhtml)
    pr.yes(len(abbreviations_list))


def render_abbreviation_entry(jinja_env, counter: int, i: dict) -> str:
...
def render_rpd_xhtml(pth: ProjectPaths, rupth: RuPaths, id_counter: int) -> int:
    ...
    rpd_letter_dict: dict = {l: [] for l in russian_alphabet}
    ...
    for lookup_entry in lookup_db:
        ...
        if first_letter not in russian_alphabet: first_letter = "а"
        rpd_entries = lookup_entry.rpd_unpack
        pali_equivalents_list = [f"<b class='epd'>{l}</b> {p}. {m}" for l, p, m in rpd_entries]
        pali_equivalents = "<br/>".join(pali_equivalents_list)
        entry = render_rpd_entry(jinja_env, id_counter, russian_headword, pali_equivalents)
    ...
    for counter, letter in enumerate(russian_alphabet):
        ...
        xhtml = render_rpd_letter_templ(jinja_env, letter, entries_str)
        output_path = rupth.epub_text_dir.joinpath(f"rpd_{counter}_{letter}.xhtml")
        with open(output_path, "w") as f: f.write(xhtml)
    pr.yes(total)
    db_session.close()
    return id_counter


def render_rpd_entry(jinja_env, counter: int, russian_headword: str, pali_equivalents: str) -> str:
    """Render single RPD entry."""
    template = jinja_env.get_template("ebook_ru_rpd_entry.jinja")
    return template.render(counter=counter, english_headword=russian_headword, pali_equivalents=pali_equivalents)


def render_rpd_letter_templ(jinja_env, letter: str, entries: str) -> str:
