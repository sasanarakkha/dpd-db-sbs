function sbs_makeFamilyCompoundHtml(data) {
  ...
  if (familyCompoundList.length > 1) {
    html += `<p class="heading" id="sbs_${lemmaTag}_cf_top">jump to: `;
    familyCompoundList.forEach((item) => {
      itemTag = item.replace(/ /g, "_");
      html += `<a class="jump" href="#sbs_${lemmaTag}_cf_${itemTag}">${item}</a> `;
    });
    html += `</p>`;
  }
  ...
  html += `
        ...
        href="https://docs.google.com/forms/d/e/1FAIpQLSf9boBe7k5tCwq7LdWgBHHGIPVc4ROO5yjVDo1X5LDAxkmGWQ/viewform?usp=pp_url&entry.438735500=${
          ...
          / /g,
          "%20",
        )}&entry.326955045=Compound+Family&entry.1433863141=GoldenDict+${data.date}"
        target="_blank">
        Fix it here</a>.`;
