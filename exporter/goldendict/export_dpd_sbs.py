"""Compile HTML data for DpdHeadword, localized for SBS study tools."""
...
from exporter.goldendict.data_classes_dps import HeadwordData
...
def render_pali_word_dpd_html(
    ...
    show_sbs_data = rd["show_sbs_data"]
    show_ru_data = rd["show_ru_data"]
    show_ta_data = rd["show_ta_data"]
    show_grammar = rd["show_grammar"]
...
def _worker_init(
    ...
    # cast: the spread literal has every DpdHeadwordRenderData key (base keys via
    # **render_data, plus pth/jinja_env), but pyrefly widens a {**TypedDict, ...}
    # spread to dict[str, ...] and cannot re-narrow it. Runtime no-op; mirrors the
    # identical construct in upstream export_dpd.py (Batch D cast precedent).
...
def _base_dpd_query(db_session: Session):
    """The joined DpdHeadword/FamilyRoot/FamilyWord query, without order or
    limit, eager-loading the Russian/Tamil/SBS translations used by the
    render step. Unlike the Russian-only export, every headword is included
    regardless of whether it has SBS/RU/TA data."""
    return (
