...
from exporter.goldendict.export_dpd_sbs import generate_dpd_html
from exporter.goldendict.export_epd_sbs import generate_epd_html
from exporter.goldendict.export_help_sbs import generate_help_html
from exporter.goldendict.export_roots_sbs import generate_root_html
...
def main():
    pr.tic()
    pr.yellow_title("exporting dpd to goldendict and mdict (sbs)")
    ...
    if g.data_limit == 0:
        root_data_list, sizes = generate_root_html(
            g.db_session, g.dpspth, g.roots_count_dict, g.show_ru_data
        )
        g.rendered_sizes.append(sizes)
