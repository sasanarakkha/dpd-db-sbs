...
1. **Repository**: Clone the DPD database repository from GitHub (using `--depth 1` for a faster download):
    ...
    cd dpd-db
    ```
2. **Database**: Download the latest `dpd.db.tar.xz` from the [dpd-db releases page](https://github.com/digitalpalidictionary/dpd-db/releases){target="_blank"} and place it in the root folder.
