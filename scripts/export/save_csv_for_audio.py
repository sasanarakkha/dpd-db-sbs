...
def main():
    ...
    clean_and_save_csv(
        os.path.join(dpspth.anki_csvs_dir, "anki_patimokkha.csv"),
        ...
        for word in words:
            if word.sbs and not SBS_table_tools().generate_sbs_audio(word.lemma_clean):
                if word.lemma_clean not in seen_palis:
                    ...
                    seen_palis.add(
                        word.lemma_clean
                    )  # Add the lemma_clean value to the set
...
def clean_and_save_csv(input_filename, output_filename):
    ...
    with open(input_filename, "r", newline="") as csvfile:
        ...
        for row in reader:
            ...
            if (
                not SBS_table_tools().generate_sbs_audio(pali)
                and pali not in seen_values
            ):
