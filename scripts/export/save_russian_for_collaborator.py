...
def save_total_ru_roots():
    ...
    dpd_db = (
        ...
        .options(joinedload(DpdHeadword.rt), joinedload(DpdHeadword.ru))
        .filter(
            DpdHeadword.ru != None,
    ...
    filtered_words = sorted(
        dpd_db,
        key=lambda word: (
            word.rt.root_count if word.rt else 0,
            word.root_key,
            word.family_root,
        ),
        reverse=True,
    ...
    with open(dpspth.ru_total_comp_path, "w", newline="") as csvfile:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            sorted_results = sorted(
                results, key=lambda x: (-x["count"], x["family_compound_word"])
            )
