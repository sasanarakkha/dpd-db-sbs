...
def ensure_directories_exist(*paths: Path, console: Console) -> None:
    ...
    for path in paths:
        ...
        if not path.exists() or not path.is_dir():
            console.print(
                f"[bold red]CRITICAL: Directory {path} was NOT created or is not a directory after attempt.[/bold red]"
            )
        else:
...
def generate_json_files(
    df: pd.DataFrame, json_dir: Path, console: Console
) -> list[dict[str, str]]:
    ...
    try:
        ...
        if not sources_json_path.exists():
            console.print(
                f"[bold red]CRITICAL: JSON file {sources_json_path} does NOT exist after write attempt.[/bold red]"
            )
        ...
        for source in sources:
            ...
            filtered_df = pd.DataFrame(source_df[columns])
            filtered_df.fillna("").to_json(
                source_json_path, force_ascii=False, orient="records", indent=2
            )
            if not source_json_path.exists():
                console.print(
                    f"[bold red]CRITICAL: Source JSON file {source_json_path} does NOT exist after write attempt.[/bold red]"
                )
...
def generate_main_html(
    pat_dir: Path, sources: list[dict[str, str]], console: Console
) -> bool:
...
def generate_source_html(
    pat_dir: Path, json_dir: Path, source: dict[str, str], console: Console
) -> bool:
...
<body>
    ...
    <h1>{source["abbrev"]}\t{source["source"]}</h1>\n""")
            ...
            sentences = pd.DataFrame(source_df[["sentence"]].drop_duplicates().ffill())
            for _, row in sentences.iterrows():
                sentence: str = str(row["sentence"])
                if sentence:
                    f.write(f"""<a href="#{sentence.replace(" ", "_")}">
                        {sentence}
                    </a><br>\n""")
            ...
            for _, sentence_row in sentences.iterrows():
                sentence: str = str(sentence_row["sentence"])
                if sentence:
                    ...
                    definitions = pd.DataFrame(
                        source_df[source_df["sentence"] == sentence][cols]
                    )
                    definitions_filled = definitions.fillna("")

                    table_html = definitions_filled.to_html(justify="left", index=False)
...
def generate_pat_links_tsv(
    tsv_path: Path, sources: list[dict[str, str]], console: Console
) -> bool:
...
def main() -> None:
    ...
    console.print(
        f"[yellow]Generating HTML pages for {len(sources)} sources...[/yellow]"
    )
    for source in sources:
