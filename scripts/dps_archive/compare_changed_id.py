...
merged_notes = pd.merge(
    file_new, file_old, on=["id", "notes"], how="outer", indicator=True
)
...
backup_ru = pd.read_csv(pth.russian_path, sep="\t")  # pyright: ignore
...
unique_ids = pd.concat([unique_to_ru, unique_to_dps])  # pyright: ignore

# Step 4: Save this list of unique IDs into a TSV file
unique_ids.to_csv(
    os.path.join(dpspth.for_compare_csv_dir, "ru_unique_ids.tsv"), sep="\t", index=False
)

print(
    f"{len(unique_ids)} IDs that do not exist in both backup_ru and backup_dps_ru saved to unique_ids.tsv"
)
...
filtered_rows[["id", "lemma_1"]].to_csv(output_path, sep="\t", index=False)  # pyright: ignore
filtered_rows_notes.to_csv(notes_output_path, sep="\t", index=False)
print(
    f"{len(filtered_rows)} words which have added new meaning saved into {output_path}"
)
print(
    f"{len(filtered_rows_notes)} words which have changed notes saved into {notes_output_path}"
)
