...
# file_to_update_path = os.path.join(dpspth.anki_csvs_dir, 'pali_class', csv_to_update)
file_to_update_path = os.path.join(dpspth.anki_csvs_dir, csv_to_update)
file_to_update = pd.read_csv(file_to_update_path, sep="\t")

file_with_user_id_path = os.path.join(
    dpspth.temp_csv_backup_dir, "paliword_user_id.tsv"
)
...
# Reorder columns
merged = merged[
    ["user_id", "id"] + [col for col in merged.columns if col not in ["user_id", "id"]]
]
