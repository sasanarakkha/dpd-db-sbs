...
def main():
    ...
    df.loc[df["link"] != "", "link"] = (
        '<a class="link" href="' + df["link"] + '">Wiki link</a>'
    )

    # Remove everything after " lit." in 'meaning_2' for the rows matching the condition
    df.loc[lit_condition_mask, "meaning_2"] = df.loc[
        lit_condition_mask, "meaning_2"
    ].apply(lambda x: x.split("; lit.")[0])
        ...
        if function_status.get(function_name, False):
            ...
            if function_name == "parittas":
                ...
                for index, row in df_result.iterrows():
                    if (
                        row["sbs_example_1"] == ""
                        and row["sbs_example_2"] == ""
                        and row["sbs_example_3"] == ""
                        and row["sbs_example_4"] == ""
                    ):
                        console.print(f"[bold red]no example for ID: {row['id']}")
...
def parittas(df, sbs_ped_link):
    ...
    filtered_df.loc[
        replacement_mask, ["sbs_source_1", "sbs_sutta_1", "sbs_example_1"]
    ] = filtered_df.loc[
        replacement_mask, ["sbs_source_2", "sbs_sutta_2", "sbs_example_2"]
    ].values
    ...
    filtered_df["feedback"] = filtered_df.apply(
        lambda row: (
...
def dps(df, dps_link):
    ...
    df = df[
        df[["sbs_example_1", "sbs_example_2", "sbs_example_3", "sbs_example_4"]].apply(
            lambda row: row.ne("").any(), axis=1
        )
    ]

    df.loc[:, "feedback"] = df.apply(
        lambda row: (
...
def dhp(df, sbs_ped_link):
    ...
    for i in range(2, 5):
        condition = (df[f"sbs_source_{i}"].str.contains("DHP", case=False)) & (
            df[f"sbs_sutta_{i}"].str.contains("vaggo", case=False)
        )
    ...
    filtered_df.loc[:, "feedback"] = filtered_df.apply(
        lambda row: (
            f"""{sbs_ped_link}={row["pali_1"]}&entry.1433863141=Parittas-{current_date}">Fix it here</a>"""
        ),
        axis=1,
...
def classes(df, sbs_ped_link):
    ...
    df.loc[mask, "ru_meaning"] = df.loc[mask, ["ru_meaning", "ru_meaning_lit"]].apply(
        lambda row: "; досл. ".join(filter(None, row)), axis=1
    )
    ...
    for sbs_class_value in unique_sbs_class_values:
        ...
        full_file_path = os.path.join(
            dpspth.anki_csvs_dir, "pali_class", "classes", file_name
        )
    ...
    output_path = os.path.join(
        dpspth.anki_csvs_dir, "pali_class", "classes", "class_total.csv"
    )
    ...
    output_path = os.path.join(
        dpspth.anki_csvs_dir, "pali_class", "classes", "class_upcoming.csv"
    )
    ...
    output_path_ru = os.path.join(dpspth.anki_csvs_dir, "pali_class", "class_ru.csv")
...
def suttas_class(df, sbs_ped_link):
    ...
    df_with_category = df_with_category[
        df_with_category["sbs_category"].str.contains("_") == False
    ]
    ...
    for category in unique_categories:
        if not pd.isnull(category) and category != "":
            ...
            output_path = os.path.join(
                dpspth.anki_csvs_dir, "pali_class", "suttas", output_filename
            )
            filtered_df.to_csv(output_path, sep="\t", index=False, header=True)
