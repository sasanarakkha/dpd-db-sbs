...
def load_processed_ids(processed_path: Path) -> Set[str]:
    ...
    Args:
        processed_path: Path to the addition_processed.json file
        ...
        for id_add, id_val in id_mapping.items():
            ...
            if id_add in line:
                ...
                if updated_line != line:
                    replacements += 1
                    print(f"Replaced '{id_add}' with '{id_val}' in {tsv_path.name} line {line_num}")
...
def process_additions() -> None:
    ...
    processed_ids = load_processed_ids(pthdps.addition_processed)
