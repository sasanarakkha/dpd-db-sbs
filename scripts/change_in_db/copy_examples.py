...
def update_column_for_some_criteria(
    ...
    console.print(
        f"[bold bright_green]Total rows already have {value_to_update}_source: {len(rows_already_have)} : "
    )

    for __word__ in rows_already_have:
        console.print(
            f"[bold bright_yellow] already {value_to_update}_source have example {__word__.id} {__word__.lemma_1}"
        )

    console.print(
        f"[bold bright_green]Total rows fit criteria sbs: {len(rows_to_update_sbs)} : "
    )

    for __word__ in rows_to_update_sbs:
        old_value = (
            getattr(__word__.sbs, column_to_update) if column_to_update else None
        )
        if not old_value or not column_to_update:
            if column_to_update:
                ...
                console.print(
                    f"[bold bright_yellow]{__word__.id} {__word__.lemma_1} {value_to_update}"
                )
            ...
            console.print(
                f"[bright_yellow]already {old_value} for {__word__.id} {__word__.lemma_1} "
            )
            count_already_have += 1

    console.print(
        f"[bold bright_green]Total rows fit criteria dpd: {len(rows_to_update)} : "
    )
    ...
    for __word__ in rows_to_update:
        if not __word__.sbs:
            ...
            if column_to_update:
                setattr(__word__.sbs, column_to_update, value_to_update)
                console.print(
                    f"[bold bright_yellow]Added {__word__.id} {__word__.lemma_1} {value_to_update}"
                )
            count_added += 1
    ...
    console.print("")
    console.print(
        f"[bold bright_green]Total count of already has: {count_already_have}"
    )
...
if __name__ == "__main__":
    ...
    update_column_for_some_criteria(
        source_value, column_to_update, value_to_update, modifier_column_to_copy
    )
