...
class JSONParser:
    ...
    def _try_direct_parse(self, content: str) -> str | None:
        """Try to parse content directly as JSON"""
        try:
            data = json.loads(content)
            if "comparisons" in data:
                pr.green("Successfully parsed pure JSON response")
                return content
        except json.JSONDecodeError:
            pass
        return None

    def _try_brace_matching(self, content: str) -> str | None:
        ...
        try:
            ...
            if first_brace != -1 and last_brace > first_brace:
                ...
                if '"comparisons"' in potential_json:
                    ...
                    pr.green("Successfully extracted JSON using brace matching")
                    return potential_json
        except json.JSONDecodeError:
            pass
        return None

    def _try_pattern_matching(self, content: str) -> str | None:
        ...
        for idx, pattern in enumerate(patterns):
            ...
            if match:
                ...
                try:
                    json.loads(potential_json)
                    pr.green(f"Pattern {idx + 1} successfully extracted JSON")
                    return potential_json
                except json.JSONDecodeError:
                    cleaned = self.clean_json_content(potential_json)
                    if cleaned:
                        pr.green(f"Pattern {idx + 1} extracted and cleaned JSON")
                        return cleaned
        return None

    def _try_enhanced_brace_matching(self, content: str) -> str | None:
                    ...
                    if char == "{":
                        ...
                        if brace_count == 0:
                            ...
                            if '"comparisons"' in potential_json:
                                json.loads(potential_json)
                                pr.green(
                                    "Successfully extracted JSON using enhanced brace matching"
                                )
                                return potential_json
                            break
        except json.JSONDecodeError:
    ...
    def extract_json_from_response(self, ai_response_content: str) -> str | None:
        ...
        try:
            ...
            if match:
                ...
                if cleaned:
                    json.loads(cleaned)
                    pr.green("Successfully extracted JSON using final fallback")
                    return cleaned
        except json.JSONDecodeError:
            pass
    ...
    def clean_json_content(self, json_content: str) -> str | None:
