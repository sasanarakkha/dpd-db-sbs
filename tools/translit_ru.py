...
def auto_translit_to_roman(text: str) -> str:
    ...
    # else convert to Roman
        ...
        try:
            ...
            print(
                f"Error during Aksharamukha transliteration: {e!s}"
            )  # Use !s for string representation
            return text
...
def main() -> None:  # Added return type hint
    ...
    test_list: List[List[str]] = [  # Added type hint
        ...
        ["buddha", "बुद्ध", "ဗුဒ္ဓ", "พุทฺธ", "බුද්ධ"],
        ["saṅgha", "सङ्घ", "သံဃ", "สงฺฆ", "සංඝ"],
        ["mettā", "메त्ता", "မေတ္တာ", "เมตฺตา", "මෙත්තා"],
        ["anicca", "अनिच्च", "အနිစ္စ", "อนิจฺจ", "අනිච්ච"],
        ["dukkha", "दुक्ख", "ဒုက္ခ", "ทุกฺข", "දුක්ඛ"],
        ["anattā", "अनत्ता", "အนตฺตา", "අනත්තා"],
        ["kamma", "कम्म", "ကမ္မ", "กมฺม", "කම්ම"],
        ["nibbāna", "निब्बान", "နිဗ္ဗාන", "นิพฺพาน", "නිබ්බාන"],
        ["sīla", "सील", "သීල", "สีล", "සීල"],
        ["samādhi", "समाधि", "သමාධි", "สมาธิ", "සමාධි"],
        ["paññā", "पञ्ञा", "ပညာ", "ปญฺญา", "පඤ්ඤා"],
        ["sutta", "суттa", "သုတ္တ", "สุตฺต", "සුත්ත"],
        ["bhikkhu", "भिक्खु", "ဘိက္ခု", "ภิกฺขุ", "භික්ඛු"],
        ["jhāna", "झान", "ဈාန", "ฌาน", "ඣාන"],
        ["vipassanā", "विपस्सना", "ဝිපဿනා", "วิปสฺสนา", "විපස්සනා"],
        ["sacca", "सच्च", "සච්ච", "සජ්ජ", "සච්ච"],
        ["vinaya", "विनय", "ဝිනය", "วินය", "විනය"],
        ["abhidhamma", "अभिधम्म", "အභිධම්ම", "อภිධමฺම", "අභිධම්ම"],
        ["karuṇā", "करुणा", "ကරුණා", "กรุณา", "කරුණා"],
        # Add a Cyrillic test case
        ["привет мир"],  # Example: "hello world" in Russian
    ]
    counter: int = 1  # Added type hint
    total_words: int = sum(
        len(sublist) for sublist in test_list
    )  # Calculate total words for counter
    ...
    for word_list in test_list:
        for word in word_list:
            ...
            pr.counter(
                counter, total_words, f"Original: '{word}', Transliterated: '{roman}'"
            )
            counter += 1
